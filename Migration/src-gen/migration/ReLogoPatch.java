...
public class ReLogoPatch extends BasePatch{
	...
	/**
	 * Sprouts (makes) a number of new migrants and then executes a set of commands on the
	 * created migrants.
	 * 
	 * @param number
	 *            a number
	...
	public AgentSet<migration.relogo.Migrant> sproutMigrants(int number, Closure closure) {
		AgentSet<migration.relogo.Migrant> result = new AgentSet<>();
		AgentSet<Turtle> createResult = this.sprout(number,closure,"Migrant");
		for (Turtle t : createResult){
	 ...
	 */
	@ReLogoBuilderGeneratedFor("migration.relogo.Migrant")
	public AgentSet<migration.relogo.Migrant> sproutMigrants(int number) {
		return sproutMigrants(number,null);
	}
